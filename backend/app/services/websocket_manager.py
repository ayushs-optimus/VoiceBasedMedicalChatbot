...
class VoiceWebSocketManager:
    """
    WebSocket manager specifically for voice chat functionality
    Can coexist with your existing WebSocket managers
    """
    ...
    async def connect(self, websocket: WebSocket, session_id: str):
        ...
        self.voice_connections[session_id] = websocket
        self.connection_metadata[session_id] = {
            "connected_at": datetime.now().isoformat(),
            "message_count": 0,
            "type": "voice"
        }
        print(f"Voice WebSocket connected for session: {session_id}")
        logger.info(f"Voice WebSocket connected for session: {session_id}")

    def disconnect(self, websocket: WebSocket, session_id: str):
        """Disconnect a voice WebSocket"""
        if session_id in self.voice_connections:
            del self.voice_connections[session_id]
    ...
    async def send_message(self, websocket: WebSocket, message: Dict):
        ...
        try:
            ...
            for session_id, ws in self.voice_connections.items():
                if ws == websocket:
                    if session_id in self.connection_metadata:
            ...
            for session_id, ws in list(self.voice_connections.items()):
                if ws == websocket:
    ...
    async def send_to_session(self, session_id: str, message: Dict):
        ...
        if session_id in self.voice_connections:
            websocket = self.voice_connections[session_id]
    ...
    async def broadcast_to_all_voice_sessions(self, message: Dict):
        ...
        for session_id, websocket in self.voice_connections.items():
            try:
                await self.send_message(websocket, message)
            except Exception as e:
    ...
    def get_active_voice_sessions(self) -> List[str]:
        ...
        return list(self.voice_connections.keys())
    ...
    def is_session_connected(self, session_id: str) -> bool:
        ...
        return session_id in self.voice_connections